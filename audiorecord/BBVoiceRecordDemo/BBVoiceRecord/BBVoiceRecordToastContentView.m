...
- (void)setUp
{
    ...
    _lbContent.frame = CGRectMake(0, 150 - textSize.height - 12, 150, textSize.height);
    NSLog(@"111===%@ self == %@",NSStringFromCGRect(_lbContent.frame),NSStringFromCGRect(self.frame) );
    
    _imgRecord.frame = CGRectMake(40, 30, _imgRecord.image.size.width, _imgRecord.image.size.height);
    _powerView.frame = CGRectMake(CGRectGetMaxY(_imgRecord.frame)+4, 30 + _imgRecord.image.size.height - powerSize.height, powerSize.width, powerSize.height);
...
- (void)setUp
{
    ...
    //    }];
    _imgRelease.frame = CGRectMake((150-_imgRelease.image.size.width)/2.0f, 30, _imgRelease.image.size.width, _imgRelease.image.size.height);
    _lbContent.frame = CGRectMake(3, (150 - 25 - 7), 150 - 10, 25);
...
- (void)updateWithRemainTime:(float)remainTime
{
    ...
    //    }];
    _lbRemainTime.frame = CGRectMake((150-textSize.width)/2.0, 16, textSize.width, 95);
}
...
- (void)showWithMessage:(NSString *)msg
{
    ...
    //        make.size.height.mas_equalTo(ceil(textSize.height));
    //    }];
    _imgIcon.frame = CGRectMake((150 - _imgIcon.image.size.width)/2.0, 32,_imgIcon.image.size.width, _imgIcon.image.size.height);
    _lbContent.frame = CGRectMake(0, 150 - textSize.height - 12, 150, textSize.height);
